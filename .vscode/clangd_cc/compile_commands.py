...
def main():
	...
	sp = subproc.run(exec_command.split(" "), stdout=subproc.PIPE, shell=True, text=True, check=True)
	if sp.returncode < 0:
	...
	compilers 	      = ['gcc', 'g++', 'as']
	compilers         = ["".join(["x86_64-w64-mingw32-", compiler]) for compiler in compilers]
	...
	for line in stdout_lines:
		which_compiler = int(line.startswith(compilers[0])) + line.startswith(compilers[1]) * 2 + line.startswith(compilers[2]) * 3
		which_compiler -= 1
	...
	for compiler in range(0, 3):
		for i in range(0, compile_lines_cnt[compiler]):
			...
			arg_str_list[0] = "".join(["/usr/bin/", compilers[compiler]])
